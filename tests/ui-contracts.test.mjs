...
test("desktop sidebar collapses to an accessible persistent navigation rail", async () => {
  ...
  assert.match(player, /className=\{`app-shell \$\{sidebarCollapsed \? "sidebar-collapsed" : ""\} \$\{nowOpen \? "now-open" : ""\} performance-\$\{resolvedAmbientQuality\}`\}/);
...
test("installed PWA checks for updates and lets the listener apply them safely", async () => {
  const [player, serviceWorker, css, packageText] = await Promise.all([
    source("../app/PlayerApp.tsx"),
    source("../public/sw.js"),
    source("../app/globals.css"),
    source("../package.json"),
  ...
  assert.match(player, /window\.addEventListener\("online", onVisibilityChange\)/);
...
test("mobile rendering is selectable instead of forcing lightweight mode on every device", async () => {
  const [component, player, css] = await Promise.all([
    source("../app/components/AnimatedAlbumBackground.tsx"),
    source("../app/PlayerApp.tsx"),
    source("../app/globals.css"),
  ...
  assert.match(component, /export function resolveAutomaticQuality\(\): ResolvedAmbientQuality/);
  assert.match(component, /navigatorWithHints\.deviceMemory <= 4/);
  assert.match(component, /navigator\.hardwareConcurrency <= 4/);
  assert.match(component, /connection\?\.saveData \|\| constrainedMemory \|\| constrainedCpu \? "low" : "high"/);
  assert.doesNotMatch(component, /mobileDevice|userAgentData|Android\|iPhone/);
  assert.match(player, /setAutomaticAmbientQuality\(resolveAutomaticQuality\(\)\)/);
  assert.match(player, /const resolvedAmbientQuality = ambientQuality === "auto" \? automaticAmbientQuality : ambientQuality/);
  assert.match(player, /performance-\$\{resolvedAmbientQuality\}/);
  assert.match(player, /ambientQuality=\{resolvedAmbientQuality\}/);
  assert.match(player, /描画パフォーマンス/);
  assert.match(player, /quality === "high" \? "標準" : "軽量"/);
  ...
  assert.match(css, /@media \(max-width:759px\) and \(pointer:coarse\)\{[\s\S]*?\.app-shell\.performance-low::before,\.app-shell\.performance-low::after\{display:none\}/);
  assert.match(css, /\.app-shell\.performance-low\.now-open>\.content,[\s\S]*?\.app-shell\.performance-low\.now-open>\.ipad-player\{visibility:hidden\}/);
  assert.match(css, /\.app-shell\.performance-low \.now-playing \.now-stage-view:not\(\.is-active\)\{visibility:hidden;content-visibility:hidden;transition:none\}/);
});
